...
### Purpose
This Version Description Document (VDD) describes the software release candidate v0.9 (test9) of the Wayside Monitor System (WMS), intended for promotion from DEV to TEST stage.

### Applicability
This document applies to the WMS release v0.9 and supersedes any prior VDD for this software baseline.
...
The release v0.9 introduces the three-class vibration anomaly classifier in anomaly-engine and the corresponding payload update in alert-dispatcher. The sensor-collector module remains unchanged at version 1.3.0.
...
The confidence field is not yet surfaced in the ops dashboard. Operators can inspect it in raw alert payloads via the syslog endpoint only. Operator run